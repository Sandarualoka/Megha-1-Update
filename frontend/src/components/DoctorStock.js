...
function DoctorStock() {
  ...
  const fetchstock = async () => {
    try {
      //   setLoading(true);
      const response = await axios.get("http://localhost:4000/get-doctorstock");
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:4000/get-all-products"
      );
  ...
  const handleUpdateStock = async () => {
    try {
      ...
      `http://localhost:4000/update-quantity/${selectedProduct}`,
      {
      method: "PUT",
      headers: {
      ...
      if (response.ok) {
        ...
        const doctorStockResponse = await fetch(
          "http://localhost:4000/create-doctorstock",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
